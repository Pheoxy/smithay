...
//!
//! Client shadows and offload framebuffers use [`transfer`] for fourcc preference and
//! operational retry (a successful lower-precision transfer is preferred over failing the
//! frame). Copy path preference is same-GPU import, then shared dmabuf / dma shadow, then
//! cpu-copy. Offload buffers are cached by size and plane fourcc; if shared import fails the
//! entry is demoted to cpu-copy until that key is invalidated.
//!
...
mod transfer;
use transfer::{
    bind_transfer_candidates, intersect_transfer_candidates, ordered_transfer_fourccs,
    try_ordered_transfer_formats,
};
...
pub struct GpuManager<A: GraphicsApi> {
    ...
    dmabuf_cache: HashMap<(DrmNode, DrmNode), Option<OffloadBuffer>>,
    span: tracing::Span,
}

/// Cached offload framebuffer allocated on the render GPU for a target output.
#[derive(Debug)]
struct OffloadBuffer {
    dmabuf: Dmabuf,
    /// When true, finish samples this buffer after import on the target; otherwise ExportMem.
    shared: bool,
    /// Plane fourcc used as part of the cache key (with buffer size).
    plane_format: Fourcc,
}
...
struct TargetData<'target, T: GraphicsApi> {
    ...
    cached_buffer: &'target mut Option<OffloadBuffer>,
    format: Fourcc,
}
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::Error: 'static,
{
        ...
        'buffer: 'frame,
    {
        let target_state = if let Some(target) = self.target.as_mut() {
            ...
            if let Some(entry) = target.cached_buffer.as_ref() {
                // The allocated buffer may use a different fourcc than the plane; key on plane.
                if entry.dmabuf.size() != buffer_size || entry.plane_format != target.format {
                    *target.cached_buffer = None;
                }
            }

            ensure_offload_framebuffer::<R, T>(buffer_size, size, dst_transform, self.render, target)?;

            let texture = resolve_offload_texture::<T>(
                target.cached_buffer.as_mut().unwrap(),
                target.device,
                buffer_size,
                *self.render.node(),
            );

            let dmabuf = &mut target.cached_buffer.as_mut().unwrap().dmabuf;
...
/// Ensure a cached offload buffer exists (shared dmabuf if possible, else cpu-copy staging).
fn ensure_offload_framebuffer<R, T: GraphicsApi>(
...
where
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::Error: 'static,
{
    if target.cached_buffer.is_some() {
        return Ok(());
    }

    match create_shared_dma_framebuffer::<R, T>(buffer_size, src, target) {
        Ok(entry) => {
            *target.cached_buffer = Some(entry);
            Ok(())
        }
        Err(err) => {
            warn!(
                "Error creating shared offload dmabuf from {} to {}: {}; falling back to cpu-copy",
                *src.node(),
                *target.device.node(),
                err
            );
            *target.cached_buffer = Some(create_cpu_copy_staging_framebuffer::<R, T>(
                buffer_size,
                size,
                dst_transform,
                src,
...
/// Import a shared offload buffer on the target, or demote the entry to cpu-copy on failure.
fn resolve_offload_texture<T: GraphicsApi>(
    entry: &mut OffloadBuffer,
    target_device: &mut T::Device,
    buffer_size: Size<i32, BufferCoords>,
    render_node: DrmNode,
) -> Option<<<T::Device as ApiDevice>::Renderer as RendererSuper>::TextureId>
where
    <T::Device as ApiDevice>::Renderer: ImportDma,
{
    if !entry.shared {
        return None;
    }
    ...
    match target_device
        .renderer_mut()
        .import_dmabuf(&entry.dmabuf, Some(&[Rectangle::from_size(buffer_size)]))
    {
        Ok(tex) => Some(tex),
        Err(err) => {
            warn!(
                ?err,
                src = %render_node,
                target = %*target_device.node(),
                "shared offload import failed; demoting to cpu-copy until cache invalidate"
            );
            entry.shared = false;
            None
        }
...
/// Allocate a shared offload dmabuf on the render GPU that the target can import.
///
/// Retries transfer fourccs when allocate, bind, or import fails.
fn create_shared_dma_framebuffer<R, T: GraphicsApi>(
    buffer_size: Size<i32, BufferCoords>,
    src: &mut R::Device,
    target: &mut TargetData<'_, T>,
) -> Result<OffloadBuffer, Error<R, T>>
where
    R: GraphicsApi + 'static,
    R::Error: 'static,
    T::Error: 'static,
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::Error: 'static,
{
    ...
    let candidates = intersect_transfer_candidates(
        &ImportDma::dmabuf_formats(target.device.renderer()),
        &Bind::<Dmabuf>::supported_formats(src.renderer()).unwrap_or_default(),
    );
    if candidates.indexset().is_empty() {
        return Err(Error::ImportFailed);
    }

    let preferred = target.format;
    let format_order =
        ordered_transfer_fourccs(None, preferred, &candidates).map_err(|()| Error::ImportFailed)?;

    match try_ordered_transfer_formats(format_order, &candidates, |code, modifiers| {
        let mut dmabuf = src
            .allocator()
            .create_buffer(buffer_size.w as u32, buffer_size.h as u32, code, modifiers)
            .map_err(|e| {
                debug!(?e, ?code, "shared offload allocate failed");
                Error::AllocatorError(e)
            })?;

        src.renderer_mut().bind(&mut dmabuf).map_err(|e| {
            debug!(?e, ?code, "shared offload bind failed");
            Error::Render(e)
        })?;
        ...
        target
            ...
            .map_err(|e| {
                debug!(?e, ?code, "shared offload import failed");
                Error::Target(e)
            })?;

        if code != preferred {
            info!(plane = ?preferred, transfer = ?code, "shared offload using degraded format");
        } else {
            info!(
        ...
        Ok(OffloadBuffer {
            dmabuf,
            shared: true,
            plane_format: preferred,
        })
    }) {
        Ok(entry) => Ok(entry),
        Err(last_err) => {
            warn!(plane = ?preferred, ?last_err, "shared offload exhausted transfer formats");
            Err(last_err.unwrap_or(Error::ImportFailed))
        }
    }
}

/// Allocate a render-only staging buffer for cpu-copy offload.
///
/// Retries transfer fourccs when allocate or bind fails.
fn create_cpu_copy_staging_framebuffer<R, T: GraphicsApi>(
    buffer_size: Size<i32, BufferCoords>,
    size: Size<i32, Physical>,
    dst_transform: Transform,
    src: &mut R::Device,
...
{
    let candidates =
        bind_transfer_candidates(Bind::<Dmabuf>::supported_formats(src.renderer()).unwrap_or_default());
    if candidates.indexset().is_empty() {
        return Err(Error::ImportFailed);
    }

    let format_order =
        ordered_transfer_fourccs(None, plane_format, &candidates).map_err(|()| Error::ImportFailed)?;
    ...
    match try_ordered_transfer_formats(format_order, &candidates, |code, modifiers| {
        let mut dmabuf = src
            ...
            .create_buffer(buffer_size.w as u32, buffer_size.h as u32, code, modifiers)
            .map_err(|e| {
                debug!(?e, ?code, "cpu-copy staging allocate failed");
                Error::AllocatorError(e)
            })?;

        // Mark as framebuffer on render first (some GL drivers dislike doing this later).
        {
            let mut fb = src.renderer_mut().bind(&mut dmabuf).map_err(|e| {
                debug!(?e, ?code, "cpu-copy staging bind failed");
                Error::Render(e)
            })?;
            src.renderer_mut()
                .render(&mut fb, size, dst_transform)
                .map_err(|e| {
                    debug!(?e, ?code, "cpu-copy staging render setup failed");
                    Error::Render(e)
                })?;
        }

        if code != plane_format {
            info!(plane = ?plane_format, transfer = ?code, "cpu-copy staging using degraded format");
        }
        ...
        Ok(entry) => Ok(entry),
        Err(last_err) => {
            warn!(plane = ?plane_format, ?last_err, "cpu-copy staging exhausted transfer formats");
            Err(last_err.unwrap_or(Error::ImportFailed))
        }
    }
}
    ...
    fn finish_internal(&mut self) -> Result<sync::SyncPoint, Error<R, T>> {
        if let Some(frame) = self.frame.take() {
            ...
            if let Some(target) = self.target.as_mut() {
                ...
                // cpu copy — ExportMem format preference: staging buffer, plane, then Abgr8888.
                let staging_format = self
                    .framebuffer
                    .as_ref()
                    .and_then(|fb| fb.format())
                    .unwrap_or(target.format);
                let mut export_formats = Vec::new();
                for code in [staging_format, target.format, Fourcc::Abgr8888] {
                    if target.device.renderer().mem_formats().any(|fmt| fmt == code)
                        && !export_formats.contains(&code)
                    {
                        export_formats.push(code);
                    }
                }
                if export_formats.is_empty() {
                    // GL always supports this for download (may truncate 10-bit).
                    export_formats.push(Fourcc::Abgr8888);
                }
                ...
                let mut last_export_err: Option<Error<R, T>> = None;
                let mut export_ok = false;
                for format in export_formats {
                    mappings.clear();
                    let mut failed = false;
                    for rect in &copy_rects {
                        match ExportMem::copy_framebuffer(
                            render.renderer_mut(),
                            self.framebuffer.as_ref().unwrap(),
                            *rect,
                            format,
                        ) {
                            Ok(mapping) => mappings.push((mapping, *rect)),
                            Err(err) => {
                                let err = Error::Render(err);
                                debug!(
                                    ?err,
                                    ?format,
                                    "cpu-copy ExportMem failed; trying next export format"
                                );
                    ...
                    if !failed {
                        if format != staging_format {
                            info!(
                                staging = ?staging_format,
                                export = ?format,
                                "cpu-copy ExportMem using degraded format"
                            );
                        }
                        export_ok = true;
                        break;
                ...
                if !export_ok {
                    warn!(
                        ?last_export_err,
                        "cpu-copy ExportMem exhausted export format preferences"
                    );
                    return Err(last_export_err.unwrap_or(Error::ImportFailed));
                }
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
{
    ...
    match try_ordered_transfer_formats(format_order, &candidates, |code, modifiers| {
        let shadow_buffer = src
            .allocator()
            .create_buffer(src_texture.width(), src_texture.height(), code, modifiers)
            .map_err(|err| {
                let err = Error::AllocatorError(err);
                debug!(
                    ?err,
                    ?code,
                    "multigpu shadow allocate failed; trying next transfer format"
                );
                err
            })?;
        ...
        let target_texture = if let Some(target) = target.as_mut() {
            match target.renderer_mut().import_dmabuf(&shadow_buffer, None) {
                Ok(tex) => Box::<<<T::Device as ApiDevice>::Renderer as RendererSuper>::TextureId>::new(tex)
            ...
            match src.renderer_mut().import_dmabuf(&shadow_buffer, None) {
                Ok(tex) => Box::<<<S::Device as ApiDevice>::Renderer as RendererSuper>::TextureId>::new(tex)
                    as Box<dyn Any + 'static>,
                Err(err) => {
                    let err = Error::Render(err);
                    debug!(
    ...
    <<<T as GraphicsApi>::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
{
    ...
    if src_node == *render.node() {
        ...
        let format = texture_internal.format;
        let size = texture_internal.size;
        ...
        if let Err(ref err) = res {
            // Rare: every transfer preference failed (see dma_shadow_copy / mem_copy).
            warn!(
                ?err,
                %src_node,
                ...
                ?format,
                ?size,
                "multigpu texture copy failed (client buffer on different GPU than render device)"
            );
        }
